...
2. Run every SQL file in `supabase/migrations/` in order in the Supabase SQL Editor. The OWASP workflow controls are in `202609200013_owasp_workflow_hardening.sql`; `202609220014_repair_update_item_status_rpc.sql` repairs the admin approval RPC and refreshes the API schema cache.
...
4. Deploy the evidence upload Edge Function with `supabase functions deploy upload-claim-evidence --project-ref YOUR_PROJECT_REF`. Redeploy it when `supabase/functions/upload-claim-evidence/index.ts` changes.
5. Copy `.env.example` to `.env.local` and set the project URL and publishable key.
6. Deploy the frontend to Vercel and add the same two environment variables in its project settings.
