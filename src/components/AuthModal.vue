<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <section role="dialog" aria-modal="true" aria-labelledby="auth-title" class="relative w-full max-w-md rounded-2xl bg-brand-paper p-6 shadow-warm-xl border border-brand-sand">
      ...
      <p class="mt-2 text-sm text-brand-mocha/75">{{ isTh ? 'กรอกอีเมล แล้วเราจะส่งลิงก์เข้าสู่ระบบที่ปลอดภัยให้คุณ' : 'Enter your email and we will send you a secure sign-in link.' }}</p>
      <form class="mt-5 space-y-3" @submit.prevent="submit">
        <label class="block text-xs font-bold text-brand-espresso" for="auth-email">Email</label>
        <input id="auth-email" v-model.trim="email" type="email" required autocomplete="email" placeholder="you@university.ac.th" class="w-full px-4 py-2.5 rounded-xl bg-brand-cream/60 border border-brand-sand text-sm focus:outline-none focus:border-brand-caramel" />
...
const allowedDomains = (import.meta.env.VITE_ALLOWED_EMAIL_DOMAINS || '').split(',').map((domain) => domain.trim().toLowerCase()).filter(Boolean)
async function submit() {
  const domain = email.value.split('@')[1]?.toLowerCase()
  if (allowedDomains.length && !allowedDomains.includes(domain)) {
    error.value = isTh.value ? 'โปรดใช้อีเมลของสถาบันที่กำหนด' : 'Please use an email from an approved institution.'
    return
  }
